...
function swiftbiz_theme_stylesheet()
{
...
add_action('wp_enqueue_scripts', 'swiftbiz_theme_stylesheet');

function swiftbiz_head() {
	global $swiftbiz_lite_shortname;
	$swiftbiz_lite_favicon = "";
	$swiftbiz_lite_meta = '<meta name="viewport" content="width=device-width; initial-scale=1.0; maximum-scale=1.0; user-scalable=0;">'."\n";

	if(swiftbiz_get_option($swiftbiz_lite_shortname.'_favicon')) {
		$swiftbiz_lite_favicon = esc_url(swiftbiz_get_option($swiftbiz_lite_shortname.'_favicon','swiftbiz-lite'));
		$swiftbiz_lite_meta .= "<link rel=\"shortcut icon\" type=\"image/x-icon\" href=\"$swiftbiz_lite_favicon\"/>\n";
	}
	echo $swiftbiz_lite_meta;

	require_once(get_template_directory().'/includes/swiftbiz-custom-css.php');
}
add_action('wp_head', 'swiftbiz_head');